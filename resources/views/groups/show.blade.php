                ...
                <div class="p-6 backdrop-blur-3xl" style="max-height: 77vh; overflow-y: auto;">
                    <div class="bg-stone-500 opacity-80 p-4 rounded-md shadow-md">
                        ...
                        <div class="hidden" id="editGroupDescForm">
                            <textarea id="newGroupDesc"
                                      class="h-32 mt-1 mb-4 w-full text-gray-600 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                      name="description" required>{{ $group->description }}</textarea>
                        </div>
                    </div>
                    <div class="bg-stone-500 opacity-80 rounded-md shadow-md px-4 pb-4 pt-1 mt-4">
                        <form method="POST" action="{{ route('groups.add', ['group' => $group]) }}">
                            ...
                            <div class="flex justify-between items-center mb-3">
                                <h3 class="text-lg text-stone-100 font-semibold mb-2 mt-4">Membri del Gruppo
                                    ({{ $group->users->count() }})</h3>
                            ...
                            @if (auth()->user()->isAdmin() && $users->count())
                                <select multiple name="users[]" id="users" multiple multiselect-search="true"
                                        multiselect-select-all="true" multiselect-max-items="4"
                                        onchange="console.log(this.selectedOptions)"
                                        class="mt-1 hidden w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            ...
                            @foreach ($group->users as $user)
                                <li class="mb-2 flex items-center">
                                    @if (auth()->user()->isAdmin() && !$user->isAdmin())
                                        <form method="POST"
                                              action="{{ route('groups.remove', ['group' => $group, 'user' => $user]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="mr-2 mt-1.5 text-red-500 hover:text-red-600 hover:rounded"
                    ...
                    <div class="flex justify-between items-center border-t-2 border-stone-400 pt-3 mt-6">
                        <p class="text-stone-400 text-sm">Data di
                            Creazione: {{ $group->created_at->format('d/m/Y') }}</p>
                        <form id="deleteGroupForm" method="POST" action="{{ route('groups.destroy', $group) }}">
                            @if (auth()->user()->isAdmin())
                                <button type="submit" id="deleteGroup" onclick="confirmDelete()"
...
<script>
    function confirmRemove(userId) {
        ...
        if (conferma) {
            document.querySelector(`form[action="/groups/{{ $group->id }}/remove-user/${u
        ) ;
